...
public class Util {
	public static Return2<String, String> getStringPart(String s) {
		...
		return new Return2<>(part.toString(), s.substring(i));
	}
	...
	public static List<DataString> toDataStringList(List<String> list)  {
		return list.stream().map(DataString::new).toList();
	}
}
